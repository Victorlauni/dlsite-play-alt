...
const isAllCatChecked = (cats: number[]): boolean => {
    return cats.length == Object.keys(WORK_CATEGORY.R18.appeal)
...
export const find = async (type: string, limit: number, offset: number): Promise<GeneralItem[]> => {
    return (await db.items.where("work_type").equals(type).reverse().sortBy("sales_date")).slice(offset, offset+limit)
}

export const filter = async (cat: number[], type: string, limit: number, offset: number): Promise<GeneralItem[]> => {
    return isAllCatChecked(cat)? await find(type, limit, offset) : await db.workCats.where("cat").anyOf(...cat).distinct().offset(offset).limit(limit).reverse().sortBy("sales_date").then(items => 
        db.items.bulkGet(items.map(work => work.workno))
    ).then(res => Promise.resolve(res.filter(it => it != undefined && it.work_type === type) as GeneralItem[]))
}
