import { Button, Checkbox, CheckboxGroup, Chip, ChipGroup, Collapse, Group, Loader, LoadingOverlay, Progress, ScrollArea, Space, Stack, Text } from '@mantine/core'
  ...
  }) 
{
  ...
  const isAllCatChecked = (): boolean => {
    return cat.length == Object.keys(WORK_CATEGORY.R18.appeal)
  ...
  const selectAllCat = () => {
    setCat(Object.keys(WORK_CATEGORY.R18.appeal)
      .concat(Object.keys(WORK_CATEGORY.R18.system))
      .concat(Object.keys(WORK_CATEGORY.R18.item))
      .concat(Object.keys(WORK_CATEGORY.R18.play))
      .concat(Object.keys(WORK_CATEGORY.R18.character))
  ...
  const updateProgress = <Stack w="100%" align='center'>
    <Loader color='blue'/>
    ...
    <ScrollArea>
      <Stack>
        ...
        <CheckboxGroup value={cat} onChange={setCat}>
          <Button fullWidth onClick={appealToggle} variant='outline'>Appeal</Button>
          <Space h={"md"}/>
          <Collapse in={appealOpen}>
            <Group pb="md">
              {Object.entries(WORK_CATEGORY.R18.appeal).map((val) => <Checkbox value={val[0]} label={val[1]} key={val[0]}/>)}
            </Group>
          </Collapse>

          <Button fullWidth onClick={systemToggle} variant='outline'>System</Button>
          <Space h={"md"}/>
          <Collapse in={systemOpen}>
            <Group pb="md">
              {Object.entries(WORK_CATEGORY.R18.system).map((val) => <Checkbox value={val[0]} label={val[1]} key={val[0]}/>)}
            </Group>
          </Collapse>

          <Button fullWidth onClick={playToggle} variant='outline'>Play</Button>
          <Space h={"md"}/>
          <Collapse in={playOpen}>
            <Group pb="md">
              {Object.entries(WORK_CATEGORY.R18.play).map((val) => <Checkbox value={val[0]} label={val[1]} key={val[0]}/>)}
            </Group>
          </Collapse>

          <Button fullWidth onClick={itemToggle} variant='outline'>Item</Button>
          <Space h={"md"}/>
          <Collapse in={itemOpen}>
            <Group pb="md">
              {Object.entries(WORK_CATEGORY.R18.item).map((val) => <Checkbox value={val[0]} label={val[1]} key={val[0]}/>)}
            </Group>
          </Collapse>

          <Button fullWidth onClick={characterToggle} variant='outline'>Character</Button>
          <Space h={"md"}/>
          <Collapse in={characterOpen}>
            <Group pb="md">
              {Object.entries(WORK_CATEGORY.R18.character).map((val) => <Checkbox value={val[0]} label={val[1]} key={val[0]}/>)}
            </Group>
          </Collapse>

          <Button fullWidth onClick={appearenceToggle} variant='outline'>Appearence</Button>
          <Space h={"md"}/>
          <Collapse in={appearenceOpen}>
            <Group pb="md">
              {Object.entries(WORK_CATEGORY.R18.appearence).map((val) => <Checkbox value={val[0]} label={val[1]} key={val[0]}/>)}
            </Group>
          </Collapse>
        </CheckboxGroup>
        <Text>Work Type</Text>
